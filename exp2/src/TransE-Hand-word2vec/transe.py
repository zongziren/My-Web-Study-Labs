...
model_entity = word2vec.Word2Vec(
    sentences, hs=1, min_count=1, window=3, size=100)

sentences = word2vec.LineSentence('relation_co.txt')
model_relation = word2vec.Word2Vec(
    sentences, hs=1, min_count=1, window=3, size=100)
    ...
    def emb_initialize(self):
        ...
        with open("entity_with_text.txt", 'r') as fr:
            for line in fr.readlines():
                ...
                for word in words:
                    vec += model_entity.wv[word]
                entity_dict[e] = vec / np.linalg.norm(vec, ord=2)

        with open("relation_with_text.txt", 'r') as fr:
            for line in fr.readlines():
                triple = line.strip().split("\t")
                e = triple[0]
                vec = 0
                words = line.strip().split('\t')[1].split(" ")
                for word in words:
                    vec += model_relation.wv[word]
                relation_dict[e] = vec / np.linalg.norm(vec, ord=2)

        for relation in self.relation:
            if relation not in relation_dict:
                r_emb_temp = np.random.uniform(-6 / math.sqrt(
                    self.embedding_dim), 6 / math.sqrt(self.embedding_dim), self.embedding_dim)
                relation_dict[relation] = r_emb_temp / \
                    np.linalg.norm(r_emb_temp, ord=2)

        for entity in self.entity:
            if entity not in entity_dict:
                e_emb_temp = np.random.uniform(-6 / math.sqrt(
                    self.embedding_dim), 6 / math.sqrt(self.embedding_dim), self.embedding_dim)
                entity_dict[entity] = e_emb_temp / \
                    np.linalg.norm(e_emb_temp, ord=2)
